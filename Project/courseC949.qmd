...
## States
  ...
  - **Bucket Sort Worst, Average and Best**
      ...
      - **Average Case:** $\text{O(n + k)}$.
      - **Worst Case:** $\text{O(n^2)}$ — All elements end up in one bucket *(degenerate case)*.
...
## States
  ...
  - **Shell Sort Worst, Average and Best**
      ...
      - **Average Case:** $\text{O(n^(3/2))}$ or $\text{O(n^1.5)}$ — Highly dependent on the gap sequence used. With commonly used sequences like the Knuth sequence, the average-case complexity is approximately $\text{O(n^1.5)}$.
...
::: {.callout-note title="Searching & Sorting Algorithms" collapse="true" appearance="simple" icon="false" style="border-left:none; border-right:none; border-top:none; border-bottom:none;"}
  ...
  - **Shell Sort:** Gap sequence, insertion-like; $\text{O(n^3/2)}$.
