...
Iterate over values
```{python iterationValue}
for value in dictionary.values():
    print(value)
```

Iterate over key-value pairs
```{python iterationPair}
for key, value in dictionary.items():
    print(f"{key}: {value}")
```

Example:
```{python iterationName}
...
Syntax:
```{python clearingFunction}
dictionary.clear()
```

Example:
```{python clearingVariable}
phone_book.clear()
```
