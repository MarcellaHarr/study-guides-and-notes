...
also be denoted by $\{ \}$. A [**finite set**]{.mark .vocab} has a finite number of elements. 
An [**infinite set**]{.mark .vocab} has an infinite number of elements.
...
A [**power set**]{.mark .vocab} is a set of all the [subsets]{.underline}. For 
example, the power set of a set $\mathbb{A}$, denoted $P\{\mathbb{A}\}$, is the 
set of all subsets of $\mathbb{A}$, so, if $\mathbb{A} = \{1,2,3\}$, then:
