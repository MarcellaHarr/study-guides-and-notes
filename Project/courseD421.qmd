...
if (!requireNamespace("vembedr", quietly = TRUE)) {
...
```{r set-basics-vid, echo=FALSE}
embed_youtube("uRj87VpsOkM") %>% 
  use_bs_responsive()
```
...
```{r identify-sets-vid, echo=FALSE}
embed_youtube("_QYbYUhFpyg") %>% 
  use_bs_responsive()
```
...
```{r intro-set-vid, echo=FALSE}
embed_youtube("UqulNbiHrrU") %>% 
  use_bs_responsive()
```
...
: Common Math Sets {#tbl-first .striped .hover tbl-colwidths="10,70,20"}
...
```{r set-builder-vide, echo=FALSE}
embed_youtube("xnfUZ-NTsCE") %>% 
  use_bs_responsive()
```
...
::: {.callout-note title="Exercises" collapse="true" appearance="simple" icon="false" style="border-left:none; border-right:none; border-top:none; border-bottom:none;"}
